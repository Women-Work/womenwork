import { Grid, Box, Button } from '@material-ui/core';
import React from 'react';
import Footer from '../../components/static/footer/Footer';
import './About.css'




function About() {
    return (
        <>
            <Grid container justifyContent='center' >
                <Grid item xs={12} sm={12} className='titulo'>
                    <h1>Women Work</h1>
                </Grid>

                <Grid item xs={12} sm={5}>
                    <h2>Quem somos</h2>
                    <p>Women Work oferece cursos dentro da plataforma que irão especializar
                        mulheres que desejam crescer no mercado de trabalho conforme a
                        necessidade de cada uma. Nosso objetivo é integrar o máximo de mulheres
                        possíveis dentro desse ambiente.</p>
                </Grid>
                <Grid item xs={12} sm={5}>
                    <h2>Nossa missão</h2>
                    <p>Promover e contribuir para o desenvolvimento profissional através
                        da educação e do trabalho alcançando o máximo de mulheres do Brasil e exterior.
                    </p>

                </Grid>
                <Grid item xs={12} sm={5}>
                    <h2>Visão</h2>
                    <p className='pad'>Planejamos estar entre as melhores e mais respeitadas
                        plataformas desse segmento por oferecer de forma consistente cursos que
                        irão inovar e agregar a vida de muitas mulheres na sua carreira profissional</p>
                </Grid>
                <Grid item xs={12} sm={5} >
                    <h2>Valores</h2>
                    <p>Procurar a melhoria contínua dos produtos e a qualidade para
                        atender às necessidades das clientes.
                        Prezamos a confiança, respeito, crença, humildade e integridade</p>
                </Grid>

            </Grid>

            <Grid container>
                <Grid item container xs={12} sm={12} justifyContent='center' className='p'>
                    <h1>Membros</h1>
                </Grid>
                <Grid item container xs={12} sm={12} className="membros" >

                    <Box>
                        <img className='img' src="https://camo.githubusercontent." alt="" />
                        <p className='nome'>Carlos Eduardo</p>
                        <Box className='button'>
                            <Button href='https://github.com/Santozcl' target={'_blank'} className="gitbutton">GitHub</Button>
                            <Button href='https://www.linkedin.com/in/carlos-eduardo-996b4421a/' target={'_blank'} className='button-linkedin'>Linkedin</Button>

                        </Box>
                    </Box>

                    <Box>
                        <img className='img' src="https://camo.githubusercontent." alt="" />
                        <p className='nome'>João Vitor</p>
                        <Box className='button'>
                            <Button href='https://github.com/Najmate' target={'_blank'} className="gitbutton">GitHub </Button>
                            <Button href='https://www.linkedin.com/in/jo%C3%A3o-vitor-estevam-82ba85207/' target={'_blank'} className='button-linkedin'>Linkedin</Button>

                        </Box>
                    </Box>


                    <Box>
                        <img className='img' src="https://camo.githubusercontent." alt="" />
                        <p className='nome' >Victor França</p>
                        <Box className='button'>
                            <Button href='https://github.com/victordnf' target={'_blank'} className="gitbutton">GitHub</Button>
                            <Button href='https://www.linkedin.com/in/victordnf/' target={'_blank'} className="button-linkedin">Linkedin</Button>

                        </Box>

                    </Box>
                    <Box>
                        <img className='img' src='https://camo.githubusercontent.' alt="" />
                        <p className='nome'>Rafaele Souza</p>
                        <Box className='button'>
                            <Button href='https://github.com/raffaez/' target={'_blank'} className="gitbutton">GitHub</Button>
                            <Button href='https://www.linkedin.com/in/rafaele-souza/' target={'_blank'} className='button-linkedin'>Linkedin</Button>

                        </Box>
                    </Box>
                    <Box>
                        <img className='img' src="https://camo.githubusercontent." alt="" />
                        <p className='nome'>Henrique Oliveira</p>
                        <Box className='button'>
                            <Button href='https://github.com/xHenrique22' target={'_blank'} className="gitbutton">GitHub</Button>
                            <Button href='https://www.linkedin.com/in/henrique-oliveira-137b1423a/' target={'_blank'} className='button-linkedin'>Linkedin</Button >

                        </Box>
                    </Box>
                    <Box>
                        <img className='img' src="https://camo.githubusercontent." alt="" />
                        <p className='nome'>Julio Cesar</p>
                        <Box className='button'>
                            <Button href='https://github.com/Juliojnr' target={'_blank'} className="gitbutton">GitHub</Button>
                            <Button href='https://www.linkedin.com/in/juliocgcj/' target={'_blank'} className="button-linkedin">Linkedin</Button>

                        </Box>

                    </Box>
                </Grid>
            </Grid>
            <Footer />
        </>
    )
}
export default About
